...
export const DivLeftStyles = styled.div<{ isOpen: boolean }>`
  transition: flex 0.2s ease-in-out;
`;

export const MainContentStyles = styled.div<{ isOpen: boolean }>`
  transition: margin-left 0.2s ease-in-out;
  margin-top: 8rem;
  padding-left: ${(props) => (props.isOpen ? "18rem" : "2rem")};
`;
...
export const NavbarTopStyles = styled.div<{ isOpen: boolean }>`
  width: 100%;
  background-color: white;
  display: flex;
  justify-content: space-between;
